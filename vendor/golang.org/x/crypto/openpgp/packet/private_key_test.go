...
func TestECDSAPrivateKey(t *testing.T) {
	...
	pirvkey1 := FromECDSA(ecdsaPriv)
	t.Logf("pirvkey1: %#x\n", pirvkey1)
	pubkey1 := elliptic.Marshal(elliptic.P256(), ecdsaPriv.PublicKey.X, ecdsaPriv.PublicKey.Y)
	t.Logf("pubkey1: %#x\n", pubkey1)

	var buf bytes.Buffer
	priv1 := NewECDSAPrivateKey(time.Now(), ecdsaPriv)
	t.Logf("priv1: %#v\n", priv1)
	if err := priv1.Serialize(&buf); err != nil {
		t.Fatal(err)
	}
	...
	t.Logf("priv2: %#v\n", priv2)
	buffer := bytes.Buffer{}
	priv2.Serialize(&buffer)
	t.Logf("%#x\n", buffer.Bytes())
	buffer.Reset()
	priv2.PublicKey.Serialize(&buffer)
	t.Logf("%#x\n", buffer.Bytes())

	pirvkey2 := FromECDSA(priv2.PrivateKey.(*ecdsa.PrivateKey))
	t.Logf("pirvkey2: %#x\n", pirvkey2)
	pubkey2 := elliptic.Marshal(elliptic.P256(), priv2.PublicKey.PublicKey.(*ecdsa.PublicKey).X, priv2.PublicKey.PublicKey.(*ecdsa.PublicKey).Y)
	t.Logf("pubkey2: %#x\n", pubkey2)
	...
	if err := sig.Sign(h, priv2, nil); err != nil {
		t.Fatal(err)
	}
	...
	if err := priv2.VerifySignature(h, sig); err != nil {
		t.Fatal(err)
	}
}
